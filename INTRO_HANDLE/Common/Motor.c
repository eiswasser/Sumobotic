...
static void MOT_PrintHelp(const CLS1_StdIOType *io) {
  ...
  CLS1_SendHelpStr((unsigned char*)"  start <number>", (unsigned char*)"Starts both engine with the given percent%\r\n", io->stdOut);
}
...
uint8_t MOT_ParseCommand(const unsigned char *cmd, bool *handled, const CLS1_StdIOType *io) {
  ...
  if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_HELP)==0 || UTIL1_strcmp((char*)cmd, (char*)"motor help")==0) {
		...
		*handled = TRUE;
	  	...
	  	MOT_StopMotor();
        *handled = TRUE;
  } else if (UTIL1_strncmp((char*)cmd, (char*)"motor start ", sizeof("motor start ")-1)==0) {
	  	p = cmd+sizeof("motor start");
